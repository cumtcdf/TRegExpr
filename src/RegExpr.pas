...
{$IFDEF OverMeth}overload; // ###0.947
  ...
  Type
    TRegexReplaceOption = (rroModifierI, rroModifierR, rroModifierS,
...
function FStrPCopy(Dest: PRegExprChar; const Source: RegExprString)
  : PRegExprChar;
var
  i, Len: integer;
begin
  ...
  Dest[Len] := #0;
  Result := Dest;
end; { of function FStrPCopy
  -------------------------------------------------------------- }

function FStrLCopy(Dest, Source: PRegExprChar; MaxLen: cardinal): PRegExprChar;
var
  i: integer;
begin
  for i := 0 to MaxLen - 1 do
    Dest[i] := Source[i];
  Result := Dest;
end; { of function FStrLCopy
  -------------------------------------------------------------- }

function FStrLen(Str: PRegExprChar): cardinal;
begin
  Result := 0;
  while Str[Result] <> #0 do
    Inc(Result);
end; { of function FStrLen
  -------------------------------------------------------------- }

function FStrPos(Str1, Str2: PRegExprChar): PRegExprChar;
var
  n: integer;
begin
  Result := nil;
  ...
  if n = 0 then
    EXIT;
  Result := Str1 + n - 1;
end; { of function FStrPos
  -------------------------------------------------------------- }

function FStrLComp(Str1, Str2: PRegExprChar; MaxLen: cardinal): integer;
var
  S1, S2: RegExprString;
begin
  S1 := Str1;
...
function FStrScan(Str: PRegExprChar; Chr: WideChar): PRegExprChar;
begin
  Result := nil;
...
function AnsiUpperCase(const s: RegExprString): RegExprString;

begin
  Result := WideUpperCase(s);
end;

function AnsiLowerCase(const s: RegExprString): RegExprString;
...
begin
  Result := {$IFDEF FPC}AnsiUpperCase(Ch)[1]{$ELSE} REChar
    (CharUpper(PChar(Ch))){$ENDIF};
  if Result = Ch then
...
{$IFDEF UniCode}
      FStrPCopy(fExpression, Copy(s, 1, Len)); // ###0.950
{$ELSE}
      FStrLCopy(fExpression, PRegExprChar(s), Len); // ###0.950
{$ENDIF UniCode}
...
begin
  Result := '';
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned(fInputString) and
    Assigned(startp[Idx]) and Assigned(endp[Idx]) and (endp[Idx] > startp[Idx])
      ...
      begin
        ...
        begin
          if (PREOp(scan)^ = EXACTLY) and
            (FStrLen(scan + REOpSz + RENextOffSz) >= PtrInt(Len)) then
          begin
...
{$ELSE}regparse^ in XIgnoredChars{$ENDIF})) then
      begin // ###0.941 \x
        ...
        begin // Skip the blanks!
          while {$IFDEF UniCode}FStrScan(XIgnoredChars, regparse^) <> nil
          // ###0.947
...
{$IFDEF UniCode}FStrScan(XIgnoredChars, regparse^) <> nil
            // ###0.947
{$ELSE}regparse^ in XIgnoredChars{$ENDIF} ) then
            EmitC(regparse^);
          Inc(regparse);
...
{$ELSE}
    ANYOF:
      while (Result < TheMax) and (FStrScan(opnd, scan^) <> nil) do
      begin
        Inc(Result);
        Inc(scan);
      end;
    ...
    case scan^ of
      ...
      BOUND:
        if (scan^ = BOUND)
          ...
          (reginput^ <> #0) and IsWordChar(reginput^) or
          (reginput <> fInputStart) and IsWordChar((reginput - 1)^) and
          ((reginput^ = #0) or not IsWordChar(reginput^))) then
          EXIT;
...
{$ELSE}
      ANYOF:
        begin
          if (reginput^ = #0) or
            (FStrScan(scan + REOpSz + RENextOffSz, reginput^) = nil) then
            EXIT;
          Inc(reginput);
        end;
      ANYBUT:
...
begin
  ...
  begin
    s := StartPtr;
    REPEAT
      s := FStrScan(s, regmust[0]);
      if s <> nil then
      begin
        if FStrLComp(s, regmust, regmlen) = 0 then
          BREAK; // Found it.
        Inc(s);
      end;
    UNTIL s = nil;
...
{$IFDEF UniCode}
  FStrPCopy(fInputString, Copy(AInputString, 1, Len)); // ###0.927
{$ELSE}
  FStrLCopy(fInputString, PRegExprChar(AInputString), Len);
{$ENDIF}
  {
    fInputString : string;
